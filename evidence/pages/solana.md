...
<div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 28px; min-height: 220px;">
  ...
  <div style="font-size: 0.85rem; color: #6b7280; margin-top: 8px;">{new Date(solana_latest[0]?.observation_date).toLocaleDateString('en-GB')}</div>
  <div style="font-size: 3.2rem; font-weight: 700; margin-top: 12px; line-height: 1;">${solana_latest[0]?.price?.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</div>
  <div style="font-size: 1rem; color: {solana_latest[0]?.daily_change >= 0 ? '#16a34a' : '#dc2626'}; margin-top: 12px;">
    {solana_latest[0]?.daily_change >= 0 ? '▲' : '▼'} {((solana_latest[0]?.daily_change ?? 0) * 100).toFixed(2)}% DoD
  </div>
</div>
{/if}
...
<div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px 24px 8px 24px;">
  ...
  <DataTable data={solana_previous}>
...
<div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 28px;">
  ...
  <BarChart data={solana_dod} x=observation_date y=daily_change yFmt=pct2 xFmt="dd/mm/yyyy" xAxisLabels=false yGridlines=false chartAreaHeight=220/>
</div>

<div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 28px;">
  <div style="font-size: 0.9rem; color: #6b7280; margin-bottom: 8px;">Close Price and Moving Averages — Last Year</div>
  <LineChart data={solana_year} x=observation_date y={['price', 'mavg_50', 'mavg_200']} xFmt="dd/mm/yyyy" yGridlines=false yScale=true chartAreaHeight=220/>
</div>
